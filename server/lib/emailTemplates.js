...
function buildVenteConfirmationEmail(firstName, vehicle, equipements, prixFinal, modePaiement) {
  const equipementsRows = (equipements || [])
    .map(e => `<tr><td style="padding:6px 0;color:#aaa;font-size:13px;font-family:monospace;">${escapeHtml(e.nom)}</td><td style="padding:6px 0;color:#fff;font-size:13px;text-align:right;font-family:monospace;">+ € ${Number(e.prix_supplement).toLocaleString('fr-FR')}</td></tr>`)
    .join('')

  const equipementsBlock = equipementsRows
    ? `<tr><td colspan="2" style="height:1px;background:#1f1f1f;"></td></tr>
       <tr><td colspan="2" style="padding:12px 0 4px;color:#00d4ff;font-size:11px;letter-spacing:2px;text-transform:uppercase;font-family:monospace;">Options</td></tr>
       ${equipementsRows}`
    : ''

  const paiementLabel = PAYMENT_METHOD_LABELS[modePaiement] || modePaiement
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr>
          <td style="background:#111;padding:40px;border:1px solid #1f1f1f;">
            ...
            <p style="margin:0 0 32px;font-size:14px;color:#aaa;line-height:1.7;font-family:monospace;">
              Bonjour <strong style="color:#fff;">${escapeHtml(firstName)}</strong>,<br>
              votre achat a été <strong style="color:#e8000d;">confirmé</strong> par notre équipe. Nous vous contacterons prochainement pour organiser la livraison.
            </p>
              ...
              <tr>
                <td style="padding:16px 24px;">
                  <table width="100%" cellpadding="0" cellspacing="0">
                    ${equipementsBlock}
                    <tr><td colspan="2" style="height:1px;background:#1f1f1f;"></td></tr>
                    <tr><td style="padding:12px 0 0;color:#888;font-size:12px;letter-spacing:2px;text-transform:uppercase;font-family:monospace;">Prix final</td><td style="padding:12px 0 0;color:#fff;font-size:16px;font-weight:900;text-align:right;font-family:monospace;">€ ${Number(prixFinal).toLocaleString('fr-FR')}</td></tr>
                    <tr><td style="padding:8px 0 0;color:#888;font-size:12px;letter-spacing:2px;text-transform:uppercase;font-family:monospace;">Paiement</td><td style="padding:8px 0 0;color:#fff;font-size:14px;text-align:right;font-family:monospace;">${escapeHtml(paiementLabel)}</td></tr>
                  </table>
                </td>
              </tr>
            </table>
